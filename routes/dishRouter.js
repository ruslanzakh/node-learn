...
dishRouter.route('/')
    ...
    .post(cors.corsWithOptions, authentic.verifyUser, authentic.verifyAdmin, (req, res, next) => {
        ...
        .then((dish) => {
            if(dish !== null && dish.comments.id(req.params.commentId) !== null) {
                if(!dish.comments.id(req.params.commentId).author._id.equals(req.user._id)) {
                    err = new Error('Comment ' + req.params.commentId + ' not yours. So you can not edit it');
                    res.statusCode = 404;
                    return next(err);
                }
                if(req.body.rating) {
...
.delete(cors.corsWithOptions, authentic.verifyUser, (req, res, next) => {
    ...
    .then((dish) => {
        if(dish !== null && dish.comments.id(req.params.commentId) !== null) {
            if(!dish.comments.id(req.params.commentId).author._id.equals(req.user._id)) {
                err = new Error('Comment ' + req.params.commentId + ' not yours. So you can not delete it');
                res.statusCode = 404;
                return next(err);
            }
